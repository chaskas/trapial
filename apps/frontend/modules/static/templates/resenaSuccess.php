<?php use_stylesheet('static/resena'); ?>
<p class="content-title">Reseña Hist&oacute;rica</p>
<div class="nuestro-jardin-wrapper">
<div id="resena-wrapper">
  <div id="texto">
    <p class="par">En el año 1995 nace el jardín Infantil  “Canta Claro”, atendiendo a niños de dos a seis años en una casa antigua en calle Orompello, en pleno centro de Concepción, que dos años después fue sustituida por un actual edificio. Esto fue una gran oportunidad para ampliar espacios y recibir a más familias, porque llegamos a una casa muy bien ubicada en calle Roosevelt, frente a Hospital Regional y Liceo Experimental. Ahí logramos consolidarnos como “Cantaclaro” y adquirir más protagonismo en el rubro ya que recibimos a recién nacidos en nuestro Nivel de Sala Cuna.  Más tarde, surge la necesidad de asociarse, en el año 2001 con Jardín Infantil “Mi pequeño Pudú” que ya atendía a hijos de funcionarios de Universidad de Concepción …</p>
    <p class="par">… Así pasó algún tiempo hasta que, con las ganas de posicionarnos con fuerzas en la ciudad, decidimos invertir en una gran casa ubicada en calle  Angol al llegar a Victor Lamas en el año 2003, donde logramos conseguir amplitud, luminosidad, magia, comodidad y nos consolidamos una vez más como uno de los Jardines Infantiles más reconocidos de la ciudad … </p>
    <p class="par">Sin embargo, el dueño decide vender terreno a una inmobiliaria que emplazó un gran edificio…, razón por la cual tuvimos que trasladarnos en el año 2005 a nuestra actual casa en calle Cochrane 870, donde llevamos 6 años y con fuertes miras de expansión en el lugar, ya que tuvimos la oportunidad de ampliar nuestra casa a Cochrane 890 y tener más salas y espacios para nuestros pequeños. Esto nos ha permitido crecer no sólo como Jardín Infantil, sino como empresa integral, ofreciendo otros servicios complementarios al rubro, para nuestra “familia Cantaclariense” esté cada día más felíz de tener un lugar que acoge con amor y vocación a sus generaciones.</p>
  </div>
  <div id="fotos1">

  </div>
</div>

<a name="peduc"></a><p class="content-title">Proyecto Educativo</p>

<div id="resena-wrapper">
  <div id="fotos2">

  </div>
  <div id="texto2">
    <ul>
      <li>
        <span class="titulo-resena">Misi&oacute;n</span>
        <p class="par">
          “Trabajar con niños y niñas de edad preescolar y sus familias, incorporando al proceso educativo <b>valores esenciales</b> para colaborar en forma activa en el encuentro y construcción de un mundo mejor”.
        </p>
        <p class="par">
          “Creamos un mundo mágico y acogedor, especializado en la primera infancia, donde el niño y la niña, de la mano con su familia y el jardín, se formen como seres integrales, ocupados de desarrollar y potenciar sus habilidades, preocupados y aprendiendo a ocuparse en el cuidado del medio ambiente y las necesidades de su entorno social.”
        </p>
      </li>
      <li>
        <span class="titulo-resena">Proyecto Metodol&oacute;gico</span>
        <p class="par">
          Nuestra institución ha escogido el tipo integral de modalidad curricular teniendo en cuenta que es la que más nos permite llevar a cabo nuestra misión, escogiendo de las otras modalidades lo que más nos acomode e identifique. Tomando siempre en cuenta algunos lineamientos filosóficos y psicológicos que son nuestro pilar.
        </p>

        <ul class="metodolo">
          <li><p class="par2">Siempre estamos dispuestos a ser una familia dispuesta a crecer y enfrentar unida los nuevos desafíos</p></li>
          <li><p class="par2">Brindamos una atención personalizada para el niño y su familia.</p></li>
          <li><p class="par2">Nuestro personal, es escogido en base a una real vocación, buscando constantemente la especialización y el perfeccionamiento continuo para la formación de nuestros pequeños.</p></li>
          <li><p class="par2">Nuestro equipo de trabajo cuenta con perfeccionamiento en Programación Neurolingüística (PNL).</p></li>
          <li><p class="par2">Buscamos que nuestros niños se expresen en forma espontánea, favoreciendo además una temprana sociabilización.</p></li>
          <li><p class="par2">Respetamos sus derechos, personalidades y características propias, así como la de sus familias.</p></li>
          <li><p class="par2">Nos preocupamos de fortalecernos en equipo, entre el personal y junto a los padres y entre los niños y niñas.</p></li>
          <li><p class="par2">Actividades recreativas, ecol&oacute;gicas.</p></li>
        </ul>

      </li>
    </ul>
  </div>
</div>
</div>