                ...
                <div id="gallery_table-content" style="min-height: 0px;">
                    ...
                    if($photos->count() > 0): ?>
                        ...
                        <div style="background: none repeat scroll 0 0 #F3F3F3; border: 1px dashed #5D5D5D; clear: both; display: none; margin-top: 15px; padding: 5px;" id="<?php echo $photo->getId(); ?>_editable" class="editable">
                            ...
                            <p>Ingrese descripci&oacute;n</p>
                            ...
                            <a href="#" onclick="ajaxPhotoEdition('<?php echo url_for('photo_ajax_default', $photo) ?>')" class="default"><img src="/sfMultipleAjaxUploadGalleryPlugin/images/starrize.png" title="Utilizar esta imagen como portada" /></a>
                            <a href="#" onclick="ajaxPhotoEdition('<?php echo url_for('photo_ajax_delete', $photo) ?>')" class="delete"><img src="/sfMultipleAjaxUploadGalleryPlugin/images/trash.png" title="Eliminar"/></a>
                        </div>
                        <?php endforeach; ?>
                        <?php foreach( $photos as $i=>$photo ): ?>
                        <div id="photo-<?php echo $photo->getId()?>" class="picture"  onclick="$(this).find('.actions').toggle();titlize('<?php echo $photo->getId(); ?>_editable');" onmouseover="$(this).find('.actions').show();" onmouseout="$(this).find('.actions').hide();">
                            <?php if($photo->getIsDefault()){ ?> <div id="default" title="Esta es usada como Portada"></div><?php } ?>
                            <img class="basic" src="/uploads/gallery/<?php echo $photo->getGalleryId()."/150/".$photo->getPicpath(); ?>"/>
                          <div class="actions">
                            <?php if(!$photo->getIsDefault()): ?>

                            <a href="#" onclick="ajaxPhotoEdition('<?php echo url_for('photo_ajax_default', $photo) ?>')" class="default"><img src="/sfMultipleAjaxUploadGalleryPlugin/images/starrize.png" align="left" title="Utilizar como portada" /></a>
                            <a href="#" onclick="ajaxPhotoEdition('<?php echo url_for('photo_ajax_delete', $photo) ?>')" class="delete"><img src="/sfMultipleAjaxUploadGalleryPlugin/images/trash.png" align="left" title="Eliminar"/></a>
                            <?php endif; ?>
                          </div>
                        </div>
                        <?php endforeach; ?>
                        <div class="clear"></div>
                        <?php else: ?>
                        <p>A&uacute;n no agregas fotos</p>
                        <?php endif; ?>
                </div>
                <!--  end table-content  -->
